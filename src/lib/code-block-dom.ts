...
/**
 * Copy text to the clipboard with a resilient two-step strategy:
 *   1. the async Clipboard API (preferred; secure contexts), then
 *   2. a hidden-`textarea` + `document.execCommand('copy')` fallback, which still
 *      works on a user gesture inside iframes (the Storyblok visual editor) and on
 *      non-secure origins, where `navigator.clipboard` is unavailable or blocked.
 * Returns `'error'` only when both paths fail.
 */
async function copyText(text: string): Promise<CopyState> {
  if (typeof navigator !== 'undefined' && navigator.clipboard?.writeText) {
    try {
      ...
      // Fall through to the legacy path (blocked iframe / permission denied).
    }
  }
  ...
  try {
    ...
    ta.remove(); // no-op if never appended; always clears the hidden node
  }
}
